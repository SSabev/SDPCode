...
void CNavigation::GenerateValues()
{
    ...
    memset(&entry->robot.sendData, 0, sizeof(TRobotData));
...
void CNavigation::GenerateMaxAngular(TEntry *entry)
{
    if((m_ourOrientation - entry->aiData.path[0].orientation) > 0){
        // turn right
        entry->robot.sendData.motor_left_speed = MAX_ONLY_ANGULAR_MSPEED;
        entry->robot.sendData.motor_right_speed = -MAX_ONLY_ANGULAR_MSPEED;
    }
    else{
        // turn left
        entry->robot.sendData.motor_left_speed = -MAX_ONLY_ANGULAR_MSPEED;
...
void CNavigation::GenerateLinear(TEntry *entry)
{
    ...
    if (eTurnLeft){
        entry->robot.sendData.motor_left_speed = (MAX_SPEED - dOmega) / SCALING_FACTOR;
        entry->robot.sendData.motor_right_speed = MAX_SPEED;
    }
    else{
        entry->robot.sendData.motor_right_speed = (MAX_SPEED - dOmega) / SCALING_FACTOR;
        entry->robot.sendData.motor_left_speed = MAX_SPEED;
    }
...
void CNavigation::GenerateAngular(TEntry *entry)
{
    ...
    if(std::abs(diff) > MAX_ALGULAR_VEL_100ms){
        if(diff > 0){
            // turn right
            entry->robot.sendData.motor_left_speed += MAX_ANGULAR_MSPEED;
            entry->robot.sendData.motor_right_speed -= MAX_ANGULAR_MSPEED;
        }
        else{
            // turn left
            entry->robot.sendData.motor_left_speed -= MAX_ANGULAR_MSPEED;
        ...
        entry->robot.sendData.motor_left_speed += diff*ANGULAR_RATIO;
        entry->robot.sendData.motor_right_speed -= diff*ANGULAR_RATIO;
    }
}
