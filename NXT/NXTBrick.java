...
public class NXTBrick implements Runnable {
  ...
  private static NXTConnection connection;
	...
	public static void main(String[] args) throws Exception {
		...
		RCXMotor ForwardMotor = new RCXMotor(MotorPort.A);
		ForwardMotor.setPower(100);
		...
		try {
			while (listening) {
				...
				if(bytes_read > 0 && !reacting) {
					...
					if(motorB_speed == 0 && motorC_speed == 0) {
						ForwardMotor.stop();
					} else {
